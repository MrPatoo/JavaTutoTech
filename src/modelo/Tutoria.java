...
public class Tutoria {
    ...
    public void GuardarTuto() {
        ...
        try {
            ...
            try {
                ...
                if (insertAuditoria != null) {
                    insertAuditoria.close();
                }
    ...
    public void cargarDatosTabla(jpAddTutoria vista) {
        ...
        if (filaSeleccionada != -1) {
            ...
            vista.txtDescripcion2.setText(DescripcionDeTB);
        }
    }
    //ACTUALIZAR/////////////////////////////////////////////////////
    ...
    public void Buscar(JTable tabla, JTextField txtBuscar) {
        ...
        modelo2.setColumnIdentifiers(new Object[] {"idTutoria", "nombreTutoria", "descripcionTutoria"});
        try {
            PreparedStatement buscarTuto = conexion.prepareStatement("SELECT * FROM tbTutoria WHERE nombreTutoria LIKE ? || '%'");
            buscarTuto.setString(1, txtBuscar.getText());
            ResultSet rs = buscarTuto.executeQuery();
 
             while (rs.next()) {
                //Llenamos el modelo por cada vez que recorremos el resultSet
                modelo2.addRow(new Object[]{rs.getString("idTutoria"), 
                    rs.getString("nombreTutoria"), 
                    rs.getString("descripcionTutoria")});
            }
            //Asignamos el nuevo modelo lleno a la tabla
