...
idTutoria VARCHAR2(100) NOT NULL,
idLeccion VARCHAR2(100) NOT NULL,
idRecordatorio VARCHAR2(100) NOT NULL,
    
CONSTRAINT FK_Role2 FOREIGN KEY (idRole) REFERENCES tbROLE(idRole),
CONSTRAINT FK_Usuario2 FOREIGN KEY (idUsuario) REFERENCES tbUsuario(idUsuario),
CONSTRAINT FK_Tutoria2 FOREIGN KEY (idTutoria) REFERENCES tbTutoria(idTutoria),
CONSTRAINT FK_Leccion2 FOREIGN KEY (idleccion) REFERENCES tbLeccion(idLeccion),
CONSTRAINT FK_Recordatorio2 FOREIGN KEY (idRecordatorio) REFERENCES tbRecordatorio(idRecordatorio)
);

--Procedimiento almacenado para eliminar datos en la tabla de auditoria
CREATE OR REPLACE PROCEDURE eliminar_auditoria (
    p_tabla_nombre in tbAuditoria.tabla_nombre%type
) AS
BEGIN
DELETE FROM tbAuditoria WHERE tabla_nombre = p_tabla_nombre;
END;

--Eliminacion de datos en la tabla de auditoria
BEGIN
eliminar_auditoria('tb');
eliminar_auditoria('');
eliminar_auditoria('');
eliminar_auditoria('');
eliminar_auditoria('');
...
--Este trigger se dispara después de una inserción en la tabla tbTutoria y registra la nueva información en la tabla auditoria.
CREATE OR REPLACE TRIGGER trigINSERT_TUTO AFTER INSERT ON tbTutoria FOR EACH ROW
BEGIN
    INSERT INTO auditoria (tabla_nombre, accion, datos_nuevos)
    VALUES ('tbTutoria', 'INSERT', TO_CLOB('idTutoria: ' || :NEW.idTutoria || ', nombreTutoria: ' || :NEW.nombreTutoria || ', descripcionTutoria: ' || :NEW.descripcionTutoria));
END;

--Este trigger se dispara después de una eliminación en la tabla tbTutoria y registra la información anterior en la tabla auditoria.
CREATE OR REPLACE TRIGGER trigDELETE_TUTO AFTER DELETE ON tbTutoria FOR EACH ROW BEGIN
    INSERT INTO auditoria (tabla_nombre, accion, datos_anteriores)
    VALUES ('tbTutoria', 'DELETE', TO_CLOB('idTutoria: ' || :OLD.idTutoria || ', nombreTutoria: ' || :OLD.nombreTutoria || ', descripcionTutoria: ' || :OLD.descripcionTutoria));
END;
